...
class GopherDirLister
  ...
  fun ref apply(dir_path: FilePath, entries: Array[String val]) =>
    ...
    for (i, entry) in entries.pairs() do
      try
        ...
        let item_type = match path_info
        | if path_info.directory => "1"
        | if path_info.file => "0"
        else
          "i"
        end
        ...
        // Hide hidden files
        let first_char = entry.substring(0, 1)
        if consume first_char != "." then
